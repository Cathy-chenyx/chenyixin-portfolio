...
- **项目背景**：GSK委托Parexel进行亚太区慢乙肝真实世界数据源的可行性评估，涉及多个国家10+个数据源，目标是筛选可用于后续RWE研究的最优数据源。

  **参与角色**：RWD战略支持

  **主要工作**：

  1. **IAR/IAI文档修订与终版交付**：根据客户comments修订IAR（Informatic Assessment Report）中main reasons部分，统一格式；更新IAI中的报价数据和患者人数信息；完成GSK项目所有版本IAR/IAI的最终归档。
  2. **患者人数与成本汇总表**：按"国家–数据源–患者人数–Total Cost–Cost per Patient"的横版结构整理，统一用成人患者口径；将Kingpoint报价按汇率（RMB→GBP）填入；整理第一轮ranking结果。
  3. **数据源报价跟踪与更新**：联仁Schewab多次报价更新，分析变化原因（数据治理费+15万/indication、新增统计分析+统计报告15万等）；Kingpoint最终报价的评估。
  4. **客户沟通邮件（英文）**：向Jaime Smith发送数据源费用调研结果和文献列表。
  5. **Roadmap与Proposal支持**：参与GSK Phase 2 proposal中RWD战略部分撰写。

  **关键成果**：GSK项目IAR/IAI于2026年3月底终版交付，项目于5月正式Close Out。客户Roadmap反馈已确认。

---

### Grifols 295372项目 — 肝硬化患者白蛋白使用RWE可行性评估

- **项目背景**：Grifols（基立福，全球血浆制品公司）委托Parexel评估在中国大陆使用真实世界数据研究肝硬化患者白蛋白使用的可行性。项目周期2026.4.15–7.15（3个月），分为Objective 1（高层面可行性）+ Objective 2（深度可行性）。

  **参与角色**：RWD战略支持 + IAP编写协助

  **主要工作**：

  1. **IAP（整合分析计划）编写**：基于Schwabe项目IAP模板，将Grifols项目变量填入；协调Medical团队（Grace）完成关键变量列表的修订与ICD-10/ATC编码整合；参与多轮内部review流程（Epi + Medical）。
  2. **文献调研**：完成15篇肝硬化+白蛋白相关文献的检索与整理；完成孙鑫教授团队（华西医院）的文献检索与RWE方法学积累调研。
  3. **数据源调研与评估**：Inspur（浪潮）因内部重组无法使用后，积极寻找替代数据源：Kingpoint（李清华教授/中山三院）、联仁（Lianren）、华西医院、青海、北方大数据、江北大数据中心等；整理数据源对比slides（含Risk Management Plan）。
  4. **关键变量对齐**：与Medical团队Grace进行变量列表对齐，将客户提供的ICD-10编码与变量列表整合，增加ATC code列，准备中文版IAI给数据源。
  5. **成本分析与谈判**：参与Pass-through Cost讨论，联仁报价从~30K USD谈判至~19K USD。
  6. **项目恢复支持**：项目On Hold后（4月28日），于5月28日正式恢复，参与时间线修订与工作重新启动。
  7. **IAP文档优化**（6月）：根据Carmen的comments更新IAP：更新start/stop date定义、添加liver transplant内容、删除冗余bullet point；与Chao讨论CAPA plan及Ginaton项目（289886）参考文档；审查IAP comments，区分clarification问题与需写入IAP的内容；向Grace咨询编码体系问题（ICD-10-CM、RxNorm、NDC在中国数据源中的适用性）。
  8. **定期会议参与**（6月5日）：参加Grifols 295372 RWD Feasibility assessment定期会议，讨论研究设计考虑、变量定义、数据分层策略等。
  ...
  **关键成果**：
  ...
  - 确认3个数据源（Lianren、Kingpoint、Data-LINK），IAI发放时间定为7月中旬
...
**项目背景**：赛诺菲委托Parexel评估亚洲地区百日咳疫苗真实世界数据的可行性，覆盖越南、菲律宾、老挝、印度四个国家。

**主要工作**：

1. **多国数据库调研**（6/1）：系统梳理四国数据库数量、数据可用性及核心发现，输出主汇总Excel（多sheet结构）
2. **越南重点数据源深入分析**：梳理VNCH医院系统、NNDSS监测系统、免疫登记系统等关键数据库，整理相关研究联系人与文献支持
3. **交付物优化**（6/2）：根据Lin反馈新增"References/URL"和"Pertussis Vaccine Type (wP/aP)"字段，对所有数据进行英文标准化处理
4. **汇报材料制作**（6/2）：制作3页英文汇报PPT，总结亚洲整体数据格局，深入展示越南重点数据资产
5. **Proposal Team对齐**（6/8）：参加赛诺菲Proposal Team对齐会议（8:30 PM旁听），跟进Kingpoint查询状态
6. **Slides Double-check**（6/12）：逐条核查Pertussis vaccine type信息，发现仅VNVC可明确为aP数据；更新slides结论

**关键成果**：调研初版已交付，获得Lin正面反馈；Proposal持续推进。
...
**Sanofi FLUHD-CNL 日本数据库评估**：
- 为赛诺菲流感住院负担研究评估日本 Medical Data Vision (MDV) 数据库可行性
- 整理 MDV Claims 数据库覆盖医院数、患者量、Disease Staging/ICD-10 编码等维度信息
- 参与跨地区供应商沟通

**MDV 合同纠纷管理**：
- 跟踪 MDV 与中国团队合同纠纷进展，参与内部沟通协调
- 更新合同条款变更方案
...
**福建大数据平台评估**：
- 参与福建省级医疗大数据平台的 RWD 可行性评估
- 调研平台数据覆盖范围、数据维度及潜在研究方向

**Wave MASH / Roche Influenza 项目**：
- 协助 Wave MASH（代谢性脂肪性肝炎）项目的文献检索与数据源评估
- 协助 Roche Influenza 项目的初步调研工作
